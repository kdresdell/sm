...
------------------------------------*/
	...
	function styledbox_shortcode( $atts, $content = null )
	{
	   extract( shortcode_atts( array(
		...
		{
		   if( !isset($style) ) $style='';
		   return '<div class="styledbox ' . esc_attr($type) .' '. esc_attr($align) .' ' . esc_attr($el_class) .'"  '. ( !empty($icon) ? 'icon' : '' ) .'" '. $style .'>'. $icon .'<div class="boxcontent">'. do_shortcode( $content ) .'<div class="clear"></div></div></div>';
		}
	}
