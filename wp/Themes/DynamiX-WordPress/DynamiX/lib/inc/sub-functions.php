<?php 
	...
	if( get_option('themeva_update') != 'updated' )
	{
		require_once NV_FILES .'/adm/inc/options-upgrade.php';
	}
	
	//delete_option( 'themeva_update' );
	...
	{ 
		...
		if( !empty($link_color_settings) )
		{
			...
			.skinset-'.$element.$z.' #content span.amount {'. $link_color_settings .' }
			';
		}
		...
		if( !empty($linkhover_color_settings) )
		{
			...
			.skinset-'.$element.$z.' .bbp-reply-content #subscription-toggle .is-subscribed a { '. $linkhover_color_settings .' }
			';	
		}
		...
		if( !empty($elem_bgcolor_settings) )
		{
			...
			$css .= $class.'.skinset-'.$element.$z.' span.nvcolor, .skinset-'.$element.$z.' span.highlight.one,.skinset-'.$element.$z.' .post-metadata li.post-date, .skinset-'.$element.$z.' .commentlist .reply a, .skinset-'.$element.$z.' div.header-infobar,.skinset-'.$element.$z.' div.item-list-tabs ul li a span,.skinset-'.$element.$z.' .trigger a, .skinset-'.$element.$z.' .widget_display_stats dd strong, .skinset-'.$element.$z.' #content #bbpress-forums ul.super-sticky,.skinset-'.$element.$z.' #content #bbpress-forums ul.sticky, .skinset-'.$element.$z.' div.bbp-template-notice, .skinset-'.$element.$z.' div#message.updated { '.$elem_bgcolor_settings.' }
			';	
		}
	
		if( !empty($elemhover_bgcolor_settings) )
		{
			$class = setcss($inherited_elements,'.nvcolor-wrap:hover span.nvcolor, .trigger a');
			$css .= $class.'.skinset-'.$element.$z.' .nvcolor-wrap:hover span.nvcolor, .skinset-'.$element.$z.' .trigger a:hover { '.$elemhover_bgcolor_settings.' }
			';	
		}
